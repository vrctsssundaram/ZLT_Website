...
// Visual layers: homepage keeps its V3 structural sheet; V4 adds the new interaction system globally.
...
// Static technical command palette
const SITE_INDEX=[
...
const needLabels={ip:'Reusable semiconductor IP',rtl:'RTL engineering',verification:'Verification closure',fpga:'FPGA proof',research:'Research collaboration'};
const stageLabels={concept:'concept / requirements',spec:'specification ready','rtl-exists':'existing RTL',stuck:'a blocked design',prototype:'prototype stage'};
const outcomeLabels={evaluation:'technical fit / evaluation',audit:'audit / readiness view',estimate:'scope and estimate',prototype:'prototype / demonstration',collaboration:'co-development discussion'};
...
function syncComposer(){const ready=Object.values(composerState).every(Boolean);if(!composerSummary||!composerGo)return;if(!ready){const missing=Object.entries(composerState).filter(([,v])=>!v).map(([k])=>k).join(' · ');composerSummary.innerHTML=`<strong>Complete the three coordinates.</strong><br/>Still needed: ${missing}`;composerGo.setAttribute('aria-disabled','true');return}const route=composerRoutes[composerState.need]||'contact.html';const qs=new URLSearchParams({stage:composerState.stage,outcome:composerState.outcome,source:'composer'});composerGo.href=route+(route.includes('?')?'&':'?')+qs.toString();composerGo.removeAttribute('aria-disabled');composerSummary.innerHTML=`<strong>${needLabels[composerState.need]}</strong> · ${stageLabels[composerState.stage]} · ${outcomeLabels[composerState.outcome]}<br/>Recommended route is ready.`;track('project_brief_composed',{...composerState})}
...
// Contextual technical-enquiry query prefill
...
const descriptionField=q('#description');if(descriptionField&&params.get('source')==='composer'){const need=params.get('service')||params.get('type')||'technical';const stage=params.get('stage')||'';const outcome=params.get('outcome')||'';descriptionField.placeholder=`Project brief route: ${need}; stage: ${stage}; desired first outcome: ${outcome}. Add the actual technical constraint, target platform or deliverable.`}
