...
themeButton?.addEventListener('click',()=>{const next=document.documentElement.dataset.theme==='dark'?'light':'dark';localStorage.setItem('zl-theme',next);applyTheme(next)});
...
// Search: intentionally static and dependency-free for GitHub Pages.
const SITE_INDEX=[
 {title:'IP Portfolio',url:'products.html',text:'13 FPGA validated semiconductor IP cores floating point IEEE 754 UART SPI I2C arithmetic interface soft IP licensing evaluation'},
 {title:'RTL Design Services',url:'services.html#rtl',text:'RTL design Verilog SystemVerilog VHDL microarchitecture lint CDC synthesis integration'},
 {title:'Verification Services',url:'services.html#verification',text:'UVM verification coverage closure regression constrained random assertions formal verification readiness audit'},
 {title:'FPGA Prototyping',url:'services.html#fpga',text:'FPGA prototyping AMD Xilinx Zynq UltraScale Alveo timing bring up bitstream'},
...
function renderSearch(term=''){if(!searchResults)return;const t=term.trim().toLowerCase();if(!t){searchResults.innerHTML='<p class="muted">Search IP, RTL, verification, FPGA, research or company information.</p>';return}const matches=SITE_INDEX.filter(x=>(x.title+' '+x.text).toLowerCase().includes(t));searchResults.innerHTML=matches.length?matches.map(x=>`<a class="search-result" href="${x.url}"><strong>${x.title}</strong><small>${x.text.split(' ').slice(0,12).join(' ')}…</small></a>`).join(''):'<p class="muted">No direct match. Try a shorter technical term.</p>'}
...
// Analytics hooks. GTM/GA can be connected later without changing page markup.
