...
LEGACY_PUBLIC_PATTERNS={
 r'assets/(?:v4|v4-pages|home-v3)\.css':'legacy stylesheet reference',
 r'class=["\'][^"\']*\bsignal-chamber\b':'legacy signal-chamber component',
 r'class=["\'][^"\']*\bsignal-topology\b':'legacy topology component',
 r'class=["\'][^"\']*\bproject-composer\b':'legacy composer component',
 r'class=["\'][^"\']*\bevidence-tape\b':'legacy evidence-tape component',
 r'class=["\'][^"\']*\bproblem-atlas\b':'legacy problem-atlas component',
 r'class=["\'][^"\']*\bcapability-field\b':'legacy capability-field component',
 r'class=["\'][^"\']*\boffer-ledger\b':'legacy offer-ledger component',
 r'class=["\'][^"\']*\bevidence-list\b':'legacy evidence-list component',
 r'class=["\'][^"\']*\bproblem-register\b':'legacy problem-register component',
 r'class=["\'][^"\']*\btheme-toggle\b':'legacy theme-toggle component',
 r'class=["\'][^"\']*\bpage-hero\b':'legacy page-hero component',
 r'class=["\'][^"\']*\bsection-code\b':'legacy section-code component',
 r'\bZepto Logic at a glance\b':'legacy at-a-glance language',
 r'>\s*Verified\s*<':'legacy Verified label',
}

class PageParser(HTMLParser):
 def __init__(self):
  super().__init__(convert_charrefs=True);self.refs=[];self.ids=set();self.images=[];self.canonical=None;self.h1_count=0;self.robots=None;self.theme=None
...
def main():
 ...
 # Staging crawl block must be explicit and complete.
 ...
 if failures:
  ...
  return 1
 print(f'PASS: V8 integrity, staging, disclosure and legacy-design guardrails clear; {len(warnings)} warning(s).')
 return 0
